...
export const TextToImageGenerator: React.FC<TextToImageGeneratorProps> = ({ onPromptGenerated, language, aiContext, ragSources, user, onAttemptGeneration, onContentGenerated }) => {
    ...
    const samplePrompts = [
        "A neon-lit cyberpunk cityscape with floating cars, vibrant purple and pink lighting, rain reflections on wet streets, futuristic architecture, high-tech aesthetic, cinematic composition, 8K resolution",
        "Ethereal forest spirit surrounded by glowing mushrooms and fireflies, magical atmosphere, soft volumetric lighting, mystical fog, ancient trees, fantasy art style, dreamy and otherworldly",
        "Retro vaporwave sunset over digital grid landscape, palm trees silhouettes, geometric mountains, pastel gradient sky, synthwave aesthetic, nostalgic 80s vibes, outrun style",
        "Abstract geometric art with flowing liquid metal textures, holographic surfaces, prismatic reflections, modern minimalist design, clean composition, premium luxury feel"
    ];

    const sampleEnhancedPrompt = "A mesmerizing cyberpunk metropolis at twilight, featuring towering skyscrapers with animated holographic advertisements casting vibrant neon reflections across rain-soaked asphalt streets. Sleek hovering vehicles navigate between illuminated glass towers while purple and magenta light beams pierce through atmospheric haze. Ultra-detailed architecture with chrome and steel surfaces, dramatic chiaroscuro lighting, cinematic wide-angle composition, photorealistic rendering with depth of field, 8K ultra-high resolution, professional digital art quality.";
...
Enhanced prompt:`;
    ...
    return (
        <div className="space-y-6">
                ...
                <div>
                    ...
                    {!prompt && (
                        <div className="mt-3 space-y-2">
                            <p className="text-xs text-gray-500">💡 Try these popular styles:</p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {samplePrompts.slice(0, 4).map((samplePrompt, index) => (
                                    <button
                                        key={index}
                                        onClick={() => setPrompt(samplePrompt)}
                                        className="text-left p-2 bg-gray-800/50 hover:bg-gray-700/50 border border-gray-600 rounded-lg text-xs text-gray-400 hover:text-gray-300 transition-colors"
                                    >
            ...
            >
                ...
                {!isLoading && !generatedImage && (
                    <div className="text-gray-500 text-center">
                        ...
                        <p className="text-xs text-gray-600 mt-1">Preview will show after clicking "Generate Image"</p>
                    </div>
                )}
            </div>
            ...
            {/* Sample Enhanced Prompt Preview - shows when no prompt has been generated */}
            {!enhancedPrompt && !isLoading && !prompt && (
                <div className="bg-gradient-to-br from-gray-800/30 to-purple-900/10 border border-gray-700/50 rounded-xl p-4 space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-bold text-gray-300">✨ Example Enhanced Prompt</h3>
                        <div className="w-2 h-2 bg-pink-400 rounded-full animate-pulse"></div>
                    </div>
                    <div className="space-y-3">
                        <div className="p-3 bg-gray-900/60 border border-gray-600/50 rounded-lg">
                            <p className="text-gray-400 text-sm font-mono leading-relaxed">
                                {sampleEnhancedPrompt}
                            </p>
                        </div>
                        <div className="flex justify-between items-center">
                            <p className="text-xs text-gray-500">
                                💡 This is what your enhanced prompt could look like
                            </p>
                            <button
                                onClick={() => navigator.clipboard.writeText(sampleEnhancedPrompt)}
                                className="flex items-center gap-1 px-2 py-1 bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 rounded-md transition-colors text-xs"
                            >
